...
function changeAddrFromMnemonic(mnemonic) {
    ...
    const account = BITBOX.HDNode.derivePath(masterHDNode, "m/44'/145'/0/0")

    // derive the first external change address HDNode which is going to spend utxo
    const change = BITBOX.HDNode.derivePath(account, "/1'")
...
async function sendBch(RECV_ADDR, SEND_ADDR, SEND_MNEMONIC) {
...
*/
export const distributeFunds = async (RECV_ADDR, walletInfo) => {
    const SEND_ADDR = walletInfo.cashAddress
    const SEND_MNEMONIC = walletInfo.mnemonic
    
    return sendBch(RECV_ADDR, SEND_ADDR, SEND_MNEMONIC);
};
