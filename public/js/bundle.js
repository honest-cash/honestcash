...
function state($stateProvider, $urlRouterProvider) {
	...
	$stateProvider.state('starter', {
		...
		resolve: {
			'notifs': function notifs($stateParams, $http, MetaService) {
		...
		controller: "feedsController",
		resolve: {
			'seo': function seo($stateParams, MetaService) {
		...
		resolve: {
			'post': function post($stateParams, $http, $q, PostService) {
				var defer = $q.defer();
...
var DraftsCtrl = function DraftsCtrl($scope, $stateParams, $http) {
    _classCallCheck(this, DraftsCtrl);
...
var EditorCtrl = function EditorCtrl($state, $scope, $stateParams, $http, $timeout, ViciAuth, API_URL) {
    ...
    var initEditor = function initEditor(postId) {
        if (!postId) {
        ...
        backgroundImageDropzone.on("sending", function (file, xhr) {
            xhr.setRequestHeader("X-Auth-Token", ViciAuth.getAuthToken());
        });

        backgroundImageDropzone.on("success", function (file, response) {
...
var FeedsCtrl = function FeedsCtrl($rootScope, $scope, $stateParams, $location, $http, FeedService, CommentService, PostService) {
	_classCallCheck(this, FeedsCtrl);
...
var ProfileCtrl = function ProfileCtrl(API_URL, $rootScope, $scope, $location, $http, $q, FeedService, CommentService, RelsService, PostService, profile) {
    ...
    $scope.fetchFeeds = function (params) {
        ...
        FeedService.fetchFeeds((_FeedService$fetchFee = {
